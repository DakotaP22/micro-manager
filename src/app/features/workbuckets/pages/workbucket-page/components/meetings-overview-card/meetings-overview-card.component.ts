...
export class MeetingsOverviewCardComponent {
  ...
  meetingTime = computed(() => {
    ...
    const d1Time = `${
      date1.getHours() > 12
        ? this.format(date1.getHours() - 12)
        : this.format(date1.getHours())
    }:${this.format(date1.getMinutes())}`;
    const d1TOD = `${date1.getHours() > 12 ? 'PM' : 'AM'}`;

    const d2Date = `${date2.getMonth()}/${date2.getDate()}/${date2.getFullYear()}`;
    const d2Time = `${
      date2.getHours() > 12
        ? this.format(date2.getHours() - 12)
        : this.format(date2.getHours())
    }:${this.format(date2.getMinutes())}`;
